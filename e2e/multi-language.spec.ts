...
test.describe('Multi-language Support', () => {
  test('should default to Ukrainian (uk)', async ({ page }) => {
    ...
    const ukrainianContent = page.locator(
      'text=/Каталог|Головна|Кошик|Контакти|Пошук|Увійти/i'
    );
    const hasUkrainian = await ukrainianContent.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should switch to English and verify content changes', async ({ page }) => {
    ...
    if (await langSwitcher.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const hasEnglish = await englishContent.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should switch to Polish and verify content changes', async ({ page }) => {
    ...
    if (await langSwitcher.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const hasPolish = await polishContent.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should persist language choice across page navigation', async ({ page }) => {
    ...
    const langSwitcher = page
      .locator(
        '[data-testid="language-switcher"], button:has-text("EN"), select[name="locale"]'
      )
      .first();
    ...
    if (await langSwitcher.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const stillEnglish = await englishContent.first().isVisible({ timeout: 5000 }).catch(() => false);
