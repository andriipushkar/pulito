...
test.describe('Product Detail Page', () => {
  test('should navigate from catalog to product', async ({ page }) => {
    ...
    await Promise.all([page.waitForURL(/\/product\//), productLink.click()]);
  ...
  test('should have related products section', async ({ page }) => {
    ...
    const relatedSection = page
      .locator('text=/схожі|рекомендо|також|related/i, [data-testid="related-products"]')
      .first();
  ...
  test('should have breadcrumbs', async ({ page }) => {
    ...
    const breadcrumb = page
      .locator(
        'nav[aria-label*="breadcrumb"], nav[aria-label*="Breadcrumb"], [class*="breadcrumb"], ol > li > a[href="/catalog"]',
      )
      .first();
