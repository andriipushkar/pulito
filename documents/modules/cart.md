...
- `role === 'wholesaler'` && `priceWholesale` -- гуртова ціна
...
Перед оформленням замовлення для гуртівників перевіряються `WholesaleRule`:
...
1. **StepContacts** -- контактна інформація (ім'я, телефон, email, компанія для гуртівників)
