...
Усі platform-keys опційні — без них фіча просто не показується в checkout. Можна задавати через `.env` або через адмінку (`/admin/payment-settings`); якщо обидва задані, **DB має пріоритет**. Apple Pay і Google Pay не мають окремих ключів — використовують WFP/LiqPay як базовий gateway.

| Змінна                       | Гайд                                               | Опис                                                             |
| ---------------------------- | -------------------------------------------------- | ---------------------------------------------------------------- |
| `LIQPAY_PUBLIC_KEY`          | [05-payment-providers.md](05-payment-providers.md) | LiqPay публічний ключ                                            |
| `LIQPAY_PRIVATE_KEY`         |                                                    | LiqPay приватний ключ (для підпису + paypart + Apple/Google Pay) |
| `MONOBANK_TOKEN`             |                                                    | Monobank Acquiring API token                                     |
| `WAYFORPAY_MERCHANT_ACCOUNT` |                                                    | WayForPay merchant account                                       |
| `WAYFORPAY_SECRET_KEY`       |                                                    | WayForPay секретний ключ (HMAC)                                  |

> Sandbox mode для LiqPay вмикається через DB-настройку `payment_liqpay_sandbox=true` в адмінці, не через env.
> "Оплата частинами" вмикається тогом `payment_liqpay_paypart_enabled=true` + кількість місяців через `payment_liqpay_paypart_count` (2–24).
...
| Змінна                   | Гайд                                                 | Опис                                                                                                               |
| ------------------------ | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `NOVA_POSHTA_API_KEY`    | [06-delivery-providers.md](06-delivery-providers.md) | API ключ Нової Пошти (v2.0); також може бути в DB через адмінку                                                    |
| `UKRPOSHTA_BEARER_TOKEN` |                                                      | Bearer токен Укрпошти (для tracking, label PDF, shipment creation). Search cities — публічний API без авторизації. |
...
| Змінна                  | За замовчуванням | Опис                                                                                                                     |
| ----------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `SENTRY_CSP_REPORT_URI` | —                | URL для Sentry CSP reporting (Content-Security-Policy violations)                                                        |
| `ADMIN_ALLOWED_IPS`     | —                | Список IP-адрес для доступу до адмін-панелі, через кому (наприклад: `1.2.3.4,5.6.7.8`). Порожнє = доступ з будь-якого IP |
| `CDN_URL`               | —                | URL CDN для статичних файлів (наприклад: `https://cdn.example.com`). Використовується для image URLs і static assets     |
