...
describe('Blog CRUD (real DB)', () => {
  ...
  it('should perform a full blog CRUD lifecycle', async () => {
    ...
    // 9. Delete the post (soft-delete: row stays, flagged deletedAt + unpublished)
    await deletePost(post.id);

    // Verify post is soft-deleted: still in DB but hidden from the site
  ...
  it('should prevent duplicate slugs', async () => {
    ...
    await expect(createCategory({ name: 'Another', slug: 'unique-cat' })).rejects.toThrow(
      /slug вже існує/i,
    );
  ...
  it('should prevent deleting a category with posts', async () => {
    ...
    await expect(deleteCategory(cat.id)).rejects.toThrow(/неможливо видалити категорію/i);
