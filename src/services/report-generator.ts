...
async function fetchFinancialReport(params: ReportParams): Promise<RowData[]> {
  ...
  const grouped = new Map<
    string,
    { revenue: number; discounts: number; delivery: number; count: number }
  >();
  for (const o of orders) {
    const dateKey = o.createdAt.toLocaleDateString('uk-UA');
...
async function fetchManagerActivity(params: ReportParams): Promise<RowData[]> {
  ...
  const stats = new Map<
    number,
    { orders: Set<number>; actions: number; statuses: Record<string, number> }
  >();
...
async function fetchAcquisitionChannels(params: ReportParams): Promise<RowData[]> {
  ...
  return Array.from(channels.entries())
    .map(([source, d]) => ({
      ...
      'Конверсія %':
        d.orders > 0 ? Number((((d.orders - d.cancelled) / d.orders) * 100).toFixed(1)) : 0,
      'Середній чек':
        d.orders - d.cancelled > 0 ? Number((d.revenue / (d.orders - d.cancelled)).toFixed(2)) : 0,
    }))
...
async function renderPdf(
  ...
  doc
    .moveTo(startX, y)
    .lineTo(startX + columns.reduce((s, c) => s + c.width + 5, 0), y)
    .stroke(BRAND.border);
  y += 8;
...
// Landscape A4: page width = 841.89, margin = 50, usable width = ~741
// Column widths are tuned so total (widths + gaps) fills the usable area
const PDF_CONFIGS: Record<TemplateKey, PdfConfig> = {
  sales_summary: {
    title: 'Звіт про продажі',
    columns: [
      { label: 'Дата', key: 'Дата', width: 130 },
      { label: 'Замовлень', key: 'Замовлень', width: 100, align: 'right' },
      { label: 'Товарів', key: 'Товарів', width: 100, align: 'right' },
      { label: 'Виручка', key: 'Виручка', width: 140, align: 'right' },
      { label: 'Середній чек', key: 'Середній чек', width: 140, align: 'right' },
    ],
  },
  products_stock: {
    ...
    columns: [
      ...
      { label: 'Назва', key: 'Назва', width: 260 },
      { label: 'Категорія', key: 'Категорія', width: 140 },
      { label: 'Залишок', key: 'Залишок', width: 70, align: 'right' },
      { label: 'Розд. ціна', key: 'Роздрібна ціна', width: 80, align: 'right' },
      { label: 'Продано', key: 'Продано (шт)', width: 70, align: 'right' },
    ],
  },
  orders_by_status: {
    title: 'Замовлення за статусом',
    columns: [
      { label: '№', key: '№ замовлення', width: 90 },
      { label: 'Дата', key: 'Дата', width: 90 },
      { label: 'Статус', key: 'Статус', width: 90 },
      { label: 'Клієнт', key: 'Клієнт', width: 160 },
      { label: 'Тип', key: 'Тип', width: 70 },
      { label: 'К-ть', key: 'Кількість товарів', width: 50, align: 'right' },
      { label: 'Сума', key: 'Сума', width: 100, align: 'right' },
    ],
  },
  clients_activity: {
    ...
    columns: [
      ...
      { label: "Ім'я", key: "Ім'я", width: 140 },
      { label: 'Телефон', key: 'Телефон', width: 120 },
      { label: 'Реєстрація', key: 'Дата реєстрації', width: 90 },
      { label: 'Замовлень', key: 'Замовлень за період', width: 80, align: 'right' },
      { label: 'Сума', key: 'Сума за період', width: 100, align: 'right' },
    ],
  },
  wholesale_report: {
    ...
    columns: [
      ...
      { label: 'Компанія', key: 'Компанія', width: 140 },
      { label: 'ЄДРПОУ', key: 'ЄДРПОУ', width: 80 },
      { label: 'Сума', key: 'Сума', width: 100, align: 'right' },
      { label: 'Оплата', key: 'Статус оплати', width: 90 },
    ],
  },
  delivery_report: {
    ...
    columns: [
      ...
      { label: 'Дата', key: 'Дата', width: 85 },
      { label: 'Клієнт', key: 'Клієнт', width: 120 },
      { label: 'Метод', key: 'Метод доставки', width: 110 },
      { label: 'Місто', key: 'Місто', width: 110 },
      { label: 'ТТН', key: 'ТТН', width: 110 },
      { label: 'Вартість', key: 'Вартість доставки', width: 80, align: 'right' },
    ],
  },
  financial_report: {
    ...
    columns: [
      ...
      { label: 'Замовлень', key: 'Замовлень', width: 75, align: 'right' },
      { label: 'Виручка', key: 'Виручка', width: 100, align: 'right' },
      { label: 'Знижки', key: 'Знижки', width: 85, align: 'right' },
      { label: 'Доставка', key: 'Доставка', width: 85, align: 'right' },
      { label: 'Чистий дохід', key: 'Чистий дохід', width: 100, align: 'right' },
      { label: 'Сер. чек', key: 'Середній чек', width: 90, align: 'right' },
    ],
  },
  returns_cancellations: {
    ...
    columns: [
      ...
      { label: 'Статус', key: 'Статус', width: 75 },
      { label: 'Клієнт', key: 'Клієнт', width: 110 },
  ...
  wholesale_groups: {
    ...
    columns: [
      ...
      { label: 'Компанія', key: 'Компанія', width: 160 },
      { label: 'Замовлень', key: 'Замовлень', width: 80, align: 'right' },
      { label: 'Сума', key: 'Сума', width: 100, align: 'right' },
      { label: 'Сер. чек', key: 'Середній чек', width: 100, align: 'right' },
    ],
  },
  product_leaders: {
    title: 'Товари-лідери та аутсайдери',
    columns: [
      { label: 'Код', key: 'Код', width: 70 },
      { label: 'Назва', key: 'Назва', width: 200 },
      { label: 'Категорія', key: 'Категорія', width: 110 },
      { label: 'Ціна', key: 'Ціна', width: 65, align: 'right' },
      { label: 'Залишок', key: 'Залишок', width: 60, align: 'right' },
      { label: 'Продано', key: 'Продано (шт)', width: 60, align: 'right' },
      { label: 'Сума', key: 'Продано (грн)', width: 80, align: 'right' },
      { label: 'Конв.%', key: 'Конверсія %', width: 55, align: 'right' },
    ],
  },
  manager_activity: {
    title: 'Активність менеджерів',
    columns: [
      { label: 'Менеджер', key: 'Менеджер', width: 130 },
      { label: 'Email', key: 'Email', width: 170 },
      { label: 'Замовлень', key: 'Оброблено замовлень', width: 80, align: 'right' },
      { label: 'Дій', key: 'Дій всього', width: 55, align: 'right' },
      { label: 'Підтв.', key: 'Підтверджено', width: 55, align: 'right' },
      { label: 'Відпр.', key: 'Відправлено', width: 55, align: 'right' },
      { label: 'Заверш.', key: 'Завершено', width: 60, align: 'right' },
      { label: 'Скасов.', key: 'Скасовано', width: 60, align: 'right' },
    ],
  },
  acquisition_channels: {
    title: 'Канали залучення',
    columns: [
      { label: 'Джерело', key: 'Джерело', width: 160 },
      { label: 'Замовлень', key: 'Замовлень', width: 90, align: 'right' },
      { label: 'Виручка', key: 'Виручка', width: 120, align: 'right' },
      { label: 'Скасовано', key: 'Скасовано', width: 80, align: 'right' },
      { label: 'Конв.%', key: 'Конверсія %', width: 80, align: 'right' },
      { label: 'Сер. чек', key: 'Середній чек', width: 100, align: 'right' },
    ],
  },
  summary_report: {
    title: 'Зведений звіт',
    columns: [
      { label: 'Показник', key: 'Показник', width: 400 },
      { label: 'Значення', key: 'Значення', width: 250, align: 'right' },
    ],
  },
  custom: {
    title: 'Власний звіт',
