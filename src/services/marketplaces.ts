...
async function withRetry<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const isRetryable =
        err instanceof Error && (err.message.includes('429') || err.message.includes('5'));
...
export async function publishToMarketplace(
  ...
  switch (channel) {
    case 'olx':
      return publishToOlx(data, appUrl);
    case 'rozetka':
      return publishToRozetka(data, appUrl);
    case 'prom':
      return publishToProm(data, appUrl);
    case 'epicentrk':
      return publishToEpicentrk(data, appUrl);
    default:
