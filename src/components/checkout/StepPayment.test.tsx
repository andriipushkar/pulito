...
describe('StepPayment', () => {
  ...
  it('shows comment error', () => {
    render(<StepPayment data={{}} errors={{ comment: 'Too long' }} onChange={vi.fn()} />);
  ...
  it('highlights selected payment method', () => {
    const { container } = render(
      <StepPayment
        data={{ paymentMethod: 'bank_transfer' as any }}
        errors={{}}
        onChange={vi.fn()}
      />,
    );
    const labels = container.querySelectorAll('label');
    const bankLabel = Array.from(labels).find((l) =>
      l.textContent?.includes('На розрахунковий рахунок'),
    );
  ...
  it('highlights selected provider', () => {
    const { container } = render(
      <StepPayment
        data={{ paymentMethod: 'online' as any, paymentProvider: 'monobank' }}
        errors={{}}
        onChange={vi.fn()}
      />,
    );
  ...
  it('calls onChange when comment textarea changes', () => {
    ...
    const { container } = render(<StepPayment data={{}} errors={{}} onChange={onChange} />);
    const textarea = container.querySelector('textarea')!;
  ...
  it('does not show providers when non-online payment selected', () => {
    ...
    const providerRadios = container.querySelectorAll(
  ...
  it('calls onChange with paymentProvider when provider radio is clicked', () => {
    ...
    // First provider is now Apple Pay (quick-checkout option promoted to top).
    fireEvent.click(providerRadios[0]);
    expect(handleChange).toHaveBeenCalledWith('paymentProvider', 'apple_pay');
  });
});
