...
export default function SubscribeButton({ productId, productName, price }: SubscribeButtonProps) {
  ...
  const handleSelectFrequency = async (frequency: 'weekly' | 'biweekly' | 'monthly' | 'bimonthly') => {
      ...
      {isOpen && (
        ...
        >
          <div className="border-b border-[var(--color-border)]/60 px-4 py-3">
            <p className="text-sm font-semibold text-[var(--color-text)]">Оберіть частоту доставки</p>
          ...
          <div className="border-t border-[var(--color-border)]/60 bg-[var(--color-bg-secondary)]/40 px-4 py-2">
            <p className="text-xs text-[var(--color-text-secondary)]">
              ...
              {' '}
              <span className="font-semibold text-green-600">Економія: {(price - discountedPrice).toFixed(2)} ₴</span>
            </p>
          </div>
        </div>
      )}
