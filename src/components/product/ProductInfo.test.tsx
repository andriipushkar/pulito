...
vi.mock('./PriceDisplay', () => ({
  default: (props: any) => <div data-testid="price-display" data-size={props.size} />,
}));
vi.mock('./QuantitySelector', () => ({
  default: ({ value, onChange, max }: any) => (
    <div data-testid="qty-selector">
      <span data-testid="qty-value">{value}</span>
      <button data-testid="qty-inc" onClick={() => onChange(Math.min(value + 1, max))}>
        +
      </button>
      <button data-testid="qty-dec" onClick={() => onChange(Math.max(value - 1, 1))}>
        -
      </button>
    </div>
  ),
...
vi.mock('./ShareButtons', () => ({
  default: (props: any) => <div data-testid="share-buttons" data-url={props.url} />,
}));
...
describe('ProductInfo', () => {
  ...
  it('shows in-stock status with quantity', () => {
    // Calm "В наявності" badge renders for stock >= 6; <= 5 uses the urgency tier.
    const { getByText } = render(<ProductInfo product={makeProduct({ quantity: 10 })} />);
    expect(getByText('В наявності (10 шт.)')).toBeInTheDocument();
  });

  it('shows low-stock urgency badge for stock <= 5', () => {
    const { getByText } = render(<ProductInfo product={makeProduct({ quantity: 5 })} />);
    expect(getByText('🔥 Поспішайте — залишилось 5 шт.')).toBeInTheDocument();
  });
  ...
  it('renders share buttons', () => {
    ...
    expect(shareButtons.some((el) => el.getAttribute('data-url') === '/product/test-product')).toBe(
      true,
    );
  });
