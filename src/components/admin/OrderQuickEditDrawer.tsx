...
export default function OrderQuickEditDrawer({ orderId, onClose, onUpdated }: Props) {
  ...
  const handleStatusChange = async () => {
    ...
    setIsSaving(false);
    if (res.success) {
      toast.success(`Статус: ${ORDER_STATUS_LABELS[newStatus as OrderStatus]}`);
  ...
  const allowedNext = (
    order ? ALLOWED_ORDER_TRANSITIONS[order.status] || [] : []
  ) as OrderStatus[];
      ...
      <div className="absolute right-0 top-0 flex h-full w-full flex-col bg-[var(--color-bg)] shadow-2xl sm:w-[440px]">
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-5 py-4">
          {order ? (
            ...
            <h3 className="text-sm font-medium">Завантаження…</h3>
          )}
          <button
            onClick={onClose}
            ...
            <div className="space-y-5">
                ...
                <ul className="space-y-1.5 rounded-lg bg-[var(--color-bg-secondary)]/40 p-3 text-xs">
                  {order.items.slice(0, 5).map((it) => (
                    <li key={it.id} className="flex justify-between gap-2">
                      ...
                      <span className="shrink-0 text-[var(--color-text-secondary)]">×{it.quantity}</span>
                    </li>
                  ))}
                  {order.items.length > 5 && (
                    <li className="text-[10px] italic text-[var(--color-text-secondary)]">
                      і ще {order.items.length - 5}…
                    </li>
                  )}
                </ul>
                <p className="mt-2 text-right text-sm font-bold">{Number(order.totalAmount).toFixed(2)} ₴</p>
              </div>
              ...
              <div>
                ...
                {allowedNext.length === 0 ? (
                  <p className="text-xs italic text-[var(--color-text-secondary)]">
                    Для статусу «{ORDER_STATUS_LABELS[order.status]}» переходи недоступні.
                  </p>
                ) : (
                  <div className="flex gap-2">
                    <Select
                      ...
                      options={[
                        ...
                        ...allowedNext.map((s) => ({ value: s, label: ORDER_STATUS_LABELS[s] })),
                      ]}
                      className="flex-1"
                    />
                    <Button size="sm" onClick={handleStatusChange} disabled={!newStatus || isSaving}>
                      Застосувати
                    </Button>
                  </div>
                )}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Видно лише команді"
