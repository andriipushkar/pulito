...
export default function UserSecurityTab({ userId }: { userId: number }) {
  const t = useTranslations('admin.userSecurityTab');
  ...
  if (isLoading)
    return <p className="text-sm text-[var(--color-text-secondary)]">{t('loading')}</p>;
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)] p-3">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('twoFactor')}</p>
          <p
            className={`mt-1 text-lg font-bold ${data.user.twoFactorEnabled ? 'text-emerald-600' : 'text-amber-600'}`}
          >
            {data.user.twoFactorEnabled ? t('enabled') : t('disabled')}
          </p>
        </div>

        <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)] p-3">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('blockStatus')}</p>
          <p
            className={`mt-1 text-lg font-bold ${data.user.isBlocked ? 'text-red-600' : 'text-emerald-600'}`}
          >
            {data.user.isBlocked ? t('blocked') : t('active')}
          </p>
          {data.user.isBlocked && data.user.blockedReason && (
            <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
              {t('reason')} {data.user.blockedReason}
            </p>
          )}
        ...
        <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)] p-3">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('lastLogin')}</p>
      ...
      <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)]">
        <div className="border-b border-[var(--color-border)] px-4 py-3">
          <h3 className="text-sm font-semibold">{t('loginHistory')}</h3>
        </div>
