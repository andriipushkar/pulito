  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-4">
        ...
        {onCloseMobile && (
          <button onClick={onCloseMobile} className="lg:hidden" aria-label="Закрити меню">
            <Close size={20} />
          </button>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3" aria-label="Головне меню">
        {sections.length === 0 && (
        ...
        {sections.map((section, si) => {
          ...
          const sectionTitle = isPinnedSection ? 'Закріплене' : section.title;
