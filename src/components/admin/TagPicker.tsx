...
export default function TagPicker({ entityType, entityId }: Props) {
  ...
  const fetchAll = async () => {
    const [tagsRes, assignedRes] = await Promise.all([
      ...
      apiClient.get<Tag[]>(`/api/v1/admin/entity-tags?entityType=${entityType}&entityId=${entityId}`),
    ]);
  ...
  const attach = async (tagId: number) => {
    ...
    if (res.success && res.data) {
      setAssigned((prev) => (prev.find((t) => t.id === res.data!.id) ? prev : [...prev, res.data!]));
    }
  };
  ...
  const createAndAttach = async () => {
    ...
    if (res.success && res.data) {
      ...
      setDraftName('');
      toast.success('Тег створено й прив\'язано');
    }
  };

  const available = allTags.filter((t) => !assigned.find((a) => a.id === t.id));
      ...
      <div className="flex flex-wrap items-center gap-1">
        {assigned.map((t) => (
          <button
            ...
            style={{
              background: t.color || 'var(--color-bg-secondary)',
              color: t.color ? '#fff' : 'var(--color-text)',
            }}
            title="Натисніть щоб відв'язати"
          >
            {t.name}
