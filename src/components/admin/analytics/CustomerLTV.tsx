...
export default function CustomerLTV({ days }: { days: number }) {
  const t = useTranslations('admin.customerLtv');
  const [data, setData] = useState<LTVData | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    apiClient
      .get<LTVData>(`/api/v1/admin/analytics/ltv?days=${days}`)
      .then((res) => {
    ...
    <div>
      <div className="mb-6 grid gap-4 md:grid-cols-4">
        <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('customers')}</p>
        ...
        <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('medianLtv')}</p>
        ...
        <div className="flex items-end gap-4" style={{ height: 160 }}>
          {data.distribution.map((bucket) => (
            <div key={bucket.label} className="flex flex-1 flex-col items-center">
              ...
              />
              <span className="mt-2 text-center text-[10px] text-[var(--color-text-secondary)]">
                {bucket.label}
              </span>
            </div>
          ))}
      ...
      {/* Top customers table */}
      <h3 className="mb-3 text-sm font-semibold">{t('topTitle')}</h3>
        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--color-bg-secondary)]">
            <tr>
              ...
              <th className="px-3 py-2 text-right">{t('colSpent')}</th>
              <th className="px-3 py-2 text-right">{t('colOrders')}</th>
              <th className="px-3 py-2 text-right">{t('colAvgCheck')}</th>
              <th className="px-3 py-2 text-right">{t('colMonthly')}</th>
              <th className="px-3 py-2 text-right">{t('colYearlyLtv')}</th>
              <th className="px-3 py-2 text-right">{t('colFirstOrder')}</th>
              <th className="px-3 py-2 text-right">{t('colLast')}</th>
            </tr>
          </thead>
          <tbody>
            {data.topCustomers.map((c, i) => (
              <tr key={c.userId} className="border-t border-[var(--color-border)]">
                ...
                <td className="px-3 py-2 text-right text-[10px] text-[var(--color-text-secondary)]">
