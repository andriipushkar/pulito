...
export default function ChannelAnalytics({ days }: { days: number }) {
  const t = useTranslations('admin.channelAnalytics');
  const SOURCE_LABELS: Record<string, string> = {
    web: t('sourceWeb'),
    telegram_bot: t('sourceTelegram'),
    viber_bot: t('sourceViber'),
  };
  const [data, setData] = useState<ChannelData | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [view, setView] = useState<
    'source' | 'utm_source' | 'utm_medium' | 'utm_campaign' | 'conversion'
  >('source');

  useEffect(() => {
    apiClient
      .get<ChannelData>(`/api/v1/admin/analytics/channels?days=${days}`)
      .then((res) => {
  ...
  const views: { key: typeof view; label: string }[] = [
    { key: 'source', label: t('viewSource') },
    { key: 'utm_source', label: 'UTM Source' },
    { key: 'utm_medium', label: 'UTM Medium' },
    { key: 'utm_campaign', label: 'UTM Campaign' },
    { key: 'conversion', label: t('viewConversion') },
  ];
  ...
  return (
    <div>
      <div className="mb-6 grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('conversionChannels')}</p>
              ...
              >
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold">
                    {t('ordersRevenue', { orders: row.orders, revenue: row.revenue.toFixed(0) })}
                  </span>
                </div>
              ...
              {(view === 'utm_source'
                ...
                return (
                  <tr key={i} className="border-t border-[var(--color-border)]">
                    <td className="px-4 py-2 text-xs font-medium">{label || t('noLabel')}</td>
      ...
      {view === 'conversion' && (
        <div className="overflow-x-auto rounded-[var(--radius)] border border-[var(--color-border)]">
          <table className="w-full text-sm">
            <thead className="bg-[var(--color-bg-secondary)]">
              <tr>
                <th className="px-4 py-2 text-left">{t('colChannel')}</th>
                <th className="px-4 py-2 text-right">{t('colVisits')}</th>
                <th className="px-4 py-2 text-right">{t('colConversions')}</th>
                <th className="px-4 py-2 text-right">{t('colConversionPct')}</th>
              </tr>
            </thead>
            <tbody>
