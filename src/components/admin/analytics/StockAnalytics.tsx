...
export default function StockAnalytics({ days }: { days: number }) {
  const t = useTranslations('admin.stockAnalytics');
  ...
  useEffect(() => {
    apiClient
      .get<StockData>(`/api/v1/admin/analytics/stock?days=${days}`)
      .then((res) => {
    ...
    <div>
      <div className="mb-6 grid gap-4 md:grid-cols-4">
        <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('activeProducts')}</p>
        ...
        <div className="rounded-[var(--radius)] border border-red-300 bg-red-50 p-4">
          <p className="text-xs text-red-600">{t('criticalStock')}</p>
        ...
        <div className="rounded-[var(--radius)] border border-orange-300 bg-orange-50 p-4">
          <p className="text-xs text-orange-600">{t('deadStockCard')}</p>
          <p className="text-2xl font-bold text-orange-700">{data.summary.deadStockCount}</p>
        </div>
        <div className="rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('avgTurnover')}</p>
      ...
      <div className="mb-4 flex gap-2">
        {(['critical', 'dead', 'turnover'] as const).map((v) => (
          ...
          >
            {v === 'critical' ? t('tabCritical') : v === 'dead' ? t('tabDead') : t('tabTurnover')}
          </button>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--color-bg-secondary)]">
            <tr>
              ...
              {view === 'critical' && (
                <>
                  <th className="px-4 py-2 text-right">{t('colSalesPerDay')}</th>
                  <th className="px-4 py-2 text-right">{t('colDaysToZero')}</th>
                </>
              )}
              {view === 'dead' && <th className="px-4 py-2 text-right">{t('colDaysNoSale')}</th>}
              {view === 'turnover' && (
                <>
                  <th className="px-4 py-2 text-right">{t('colSold')}</th>
                  <th className="px-4 py-2 text-right">{t('colTurnover')}</th>
                </>
              )}
            </tr>
          </thead>
                  ...
                  <td className="px-4 py-2 text-right">
                    <span
                      className={`rounded px-1.5 py-0.5 text-xs font-medium ${
                        (item.daysUntilOut ?? 999) <= 3
                          ? 'bg-red-100 text-red-700'
                          : (item.daysUntilOut ?? 999) <= 7
                            ? 'bg-orange-100 text-orange-700'
                            : 'bg-yellow-100 text-yellow-700'
                      }`}
                    >
                      {item.daysUntilOut != null
                        ? t('daysShort', { days: item.daysUntilOut })
                        : '—'}
                    </span>
                  </td>
                </tr>
                ...
                <tr key={item.id} className="border-t border-[var(--color-border)]">
                  ...
                  <td className="px-4 py-2 text-right">
                    <span className="rounded bg-orange-100 px-1.5 py-0.5 text-xs font-medium text-orange-700">
                      {item.daysSinceLastSale ?? t('never')}
                    </span>
                  </td>
                </tr>
              ))}
            {view === 'turnover' &&
              data.turnoverRates.map((item) => (
                <tr key={item.id} className="border-t border-[var(--color-border)]">
                  <td className="px-4 py-2 text-xs font-mono">{item.code}</td>
                  <td className="px-4 py-2 text-xs">{item.name}</td>
                  <td className="px-4 py-2 text-right text-xs">{item.quantity}</td>
