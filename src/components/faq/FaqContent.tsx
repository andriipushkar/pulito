  ...
  const renderFaqList = (items: FaqItem[], query?: string) => (
    <Accordion>
      {items.map((item) => {
        const title = query ? (
          <span
            dangerouslySetInnerHTML={{ __html: highlightText(sanitizeHtml(item.question), query) }}
          />
        ) : (
          item.question
        );
    ...
    <div>
      ...
      <div className="mt-10 rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-6 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-3">
          ...
          <a
            href="viber://pa?chatURI=pulito_trade"
