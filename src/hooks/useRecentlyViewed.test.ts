...
describe('useRecentlyViewed', () => {
  ...
  it('loads ids from localStorage', () => {
    localStorage.setItem('pulito-recently-viewed', JSON.stringify([5, 3, 1]));
    const { result } = renderHook(() => useRecentlyViewed());
  ...
  it('addItem adds a product id to the beginning', () => {
    ...
    expect(JSON.parse(localStorage.getItem('pulito-recently-viewed')!)).toEqual([10]);
  });

  it('addItem moves existing id to the front', () => {
    localStorage.setItem('pulito-recently-viewed', JSON.stringify([1, 2, 3]));
    const { result } = renderHook(() => useRecentlyViewed());
  ...
  it('addItem limits to MAX_ITEMS (15)', () => {
    ...
    localStorage.setItem('pulito-recently-viewed', JSON.stringify(existing));
    const { result } = renderHook(() => useRecentlyViewed());
  ...
  it('getItems returns current ids', () => {
    localStorage.setItem('pulito-recently-viewed', JSON.stringify([7, 8]));
    const { result } = renderHook(() => useRecentlyViewed());
  ...
  it('handles invalid localStorage data gracefully', () => {
    localStorage.setItem('pulito-recently-viewed', 'invalid-json');
    const { result } = renderHook(() => useRecentlyViewed());
    expect(result.current.ids).toEqual([]);
  });
  ...
  it('merges localStorage with server when user logs in', async () => {
    ...
    // Wait for effect
    await vi.waitFor(() => {
      expect(mockPost).toHaveBeenCalledWith('/api/v1/me/recently-viewed/merge', {
        productIds: [1, 2, 3],
      });
    });
  });
  ...
  it('skips merge when localStorage has empty array', async () => {
    localStorage.setItem('pulito-recently-viewed', JSON.stringify([]));
    mockUser.current = { id: 1 };
    ...
    expect(mockPost).not.toHaveBeenCalledWith(
      '/api/v1/me/recently-viewed/merge',
      expect.anything(),
    );
  });
  ...
  it('handles merge API failure gracefully (catch path)', async () => {
    localStorage.setItem('pulito-recently-viewed', JSON.stringify([10, 20]));
    mockUser.current = { id: 1 };
    ...
    expect(localStorage.getItem('pulito-recently-viewed')).not.toBeNull();
  });

  it('removes localStorage after successful merge', async () => {
    localStorage.setItem('pulito-recently-viewed', JSON.stringify([5, 6]));
    mockUser.current = { id: 1 };
    mockPost.mockResolvedValue({ success: true });
    ...
    await vi.waitFor(() => {
      expect(localStorage.getItem('pulito-recently-viewed')).toBeNull();
    });
  });
});
