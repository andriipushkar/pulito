    ...
    vi.mocked(prisma.product.findMany).mockResolvedValue([
      {
        ...
        category: { name: 'Cat', slug: 'cat', parent: null },
        images: [],
      },
    ] as any);
    ...
    vi.mocked(prisma.product.findMany).mockResolvedValue([
      {
        ...
        updatedAt: new Date(),
        content: null,
        category: { name: 'Cat', slug: 'cat', parent: { name: 'Parent' } },
        images: [{ pathFull: '/img/sale.jpg' }],
      },
    ] as any);
    const res = await GET(fakeReq());
    expect(res.status).toBe(200);
    ...
    vi.mocked(prisma.product.findMany).mockResolvedValue([
      {
        ...
        updatedAt: new Date(),
        content: null,
        category: { name: 'Single', slug: 'single', parent: null },
        images: [],
      },
    ] as any);
    const res = await GET(fakeReq());
