...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  if (!raw) return { title: 'Товар не знайдено' };
  // Substitute EN fields when locale=en so og/twitter/title use the right
  // language. uk passes through unchanged.
  const product = applyTranslationsDeep(raw, locale)!;
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: buildHreflang(`/product/${slug}`),
    },
    openGraph: {
      title,
      description,
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const relatedRaw = product.category
    ...
    : [];
  const relatedProducts = applyTranslationsList(relatedRaw, locale);
