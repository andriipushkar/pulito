...
export async function generateMetadata({ params }: BundleDetailPageProps): Promise<Metadata> {
  ...
  const raw = await getBundleBySlug(slug);
  if (!raw) return { title: 'Комплект не знайдено' };
  const bundle = applyTranslationsDeep(raw, locale)!;

  const baseUrl = process.env.APP_URL || 'http://localhost:3000';
  const url = `${baseUrl}/bundles/${slug}`;
  const title = `${bundle.name} — Комплект`;
  const description = bundle.description || `Комплект "${bundle.name}" — купити за вигідною ціною`;
  // Prefer the bundle's own image; fall back to the site's OG card so social
  // previews never render with a blank thumbnail.
  const ogImageUrl = bundle.imagePath
    ? `${baseUrl}${bundle.imagePath}`
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: buildHreflang(`/bundles/${slug}`),
    },
    openGraph: {
      title,
      description,
