  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Snapshot what was submitted into `data` so the read-only "your request"
        // view (rendered when status === 'pending') shows the values the user
        // just sent, not the previous draft.
        setData((prev) =>
          prev
            ? {
                ...prev,
                ...
                taxSystem,
                legalAddress,
                contactPersonName: contactName || user?.fullName || prev.contactPersonName,
                contactPersonPhone: cleanPhone(contactPhone),
                wholesaleMonthlyVol: monthlyVol,
              }
            : prev,
        );
        // Reset the editable form so it isn't filled with stale text if the
        // user comes back (e.g. after rejection) to submit a new request.
        setCompanyName('');
        setEdrpou('');
        setOwnershipType('');
        setTaxSystem('');
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Company info */}
        <div
          className={`rounded-2xl border border-[var(--color-border)]/60 bg-[var(--color-bg)] p-6 ${currentStep === 1 ? '' : 'hidden'}`}
        >
        ...
        {/* Contact person */}
        <div
          className={`rounded-2xl border border-[var(--color-border)]/60 bg-[var(--color-bg)] p-6 ${currentStep === 2 ? '' : 'hidden'}`}
        >
          <div className="mb-5 flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-primary)]/10 text-[var(--color-primary)]">
              <svg
        ...
        {/* Additional */}
        <div
          className={`rounded-2xl border border-[var(--color-border)]/60 bg-[var(--color-bg)] p-6 ${currentStep === 3 ? '' : 'hidden'}`}
        >
          <div className="mb-5 flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-primary)]/10 text-[var(--color-primary)]">
              <svg
