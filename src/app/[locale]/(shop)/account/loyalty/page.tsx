...
export default function LoyaltyPage() {
  ...
  if (!data) {
    return (
      <p className="text-sm text-[var(--color-text-secondary)]">Не вдалося завантажити дані</p>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={
          currentLevel
            ? `Рівень: ${account.level} — знижка ${currentLevel.discountPercent}%`
            : undefined
        }
      />
      ...
      <div
        className={`relative overflow-hidden rounded-2xl border border-l-4 p-5 sm:p-6 ${LEVEL_BG[account.level] || 'bg-blue-50'} border-opacity-30`}
        style={{ borderColor: levelColor, borderLeftColor: levelColor }}
      >
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div
              className={`flex h-14 w-14 items-center justify-center rounded-2xl ${LEVEL_BG[account.level] || 'bg-blue-50'}`}
            ...
            <div>
              ...
              <p className="text-xl font-bold capitalize" style={{ color: levelColor }}>
                {account.level}
              </p>
              {currentLevel && (
                <p className="text-xs text-[var(--color-text-secondary)]">
                  Знижка {currentLevel.discountPercent}% &middot; Множник x
                  {currentLevel.pointsMultiplier}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mt-5 border-t border-[var(--color-border)]/40 pt-4">
            <div className="mb-2 flex items-baseline justify-between gap-2">
              <span className="text-xs font-medium text-[var(--color-text-secondary)]">
                До рівня{' '}
                <strong className="capitalize text-[var(--color-text)]">{nextLevel.name}</strong>
              </span>
              <span className="text-xs font-medium text-[var(--color-text-secondary)]">
                {account.totalSpent.toFixed(0)} / {Number(nextLevel.minSpent).toFixed(0)} ₴ &middot;{' '}
      ...
      <div>
        ...
        <div className="overflow-hidden rounded-2xl border border-[var(--color-border)]/60 bg-[var(--color-bg)]">
          {recentTransactions.map((tx, idx) => (
            <div
              key={tx.id}
              className={`group flex items-center justify-between px-4 py-3${idx < recentTransactions.length - 1 ? ' border-b border-[var(--color-border)]/60' : ''}`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`flex h-8 w-8 items-center justify-center rounded-lg ${tx.points >= 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}
                >
                  <svg
                    className="h-4 w-4"
                    fill="none"
                  ...
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d={TYPE_ICONS[tx.type] || 'M12 4v16m8-8H4'}
                    />
                  </svg>
                </div>
                <div>
                  ...
                  <p className="text-xs text-[var(--color-text-secondary)]">
                    {TYPE_LABELS[tx.type] || tx.type} &middot;{' '}
