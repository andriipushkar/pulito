...
export default function AddressesPage() {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      label: trimOrUndef(form.label),
      city,
      street: trimOrUndef(form.street),
      building: trimOrUndef(form.building),
      apartment: trimOrUndef(form.apartment),
  ...
  const handleDelete = async (id: number) => {
    ...
    const res = await apiClient.delete(`/api/v1/me/addresses/${id}`);
    if (res.success) {
      toast.success('Адресу видалено');
      await load();
    } else {
      toast.error(res.error || 'Не вдалося видалити адресу');
    }
  };

  const handleSetDefault = async (id: number) => {
    const res = await apiClient.put(`/api/v1/me/addresses/${id}`, { isDefault: true });
    if (res.success) {
      toast.success('Адресу за замовчуванням оновлено');
      await load();
    } else {
      toast.error(res.error || 'Не вдалося оновити адресу за замовчуванням');
    }
  };
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex shrink-0 gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                  ...
                  <button
                    ...
                    className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-[var(--color-primary)] hover:bg-blue-50"
                  >
                    <svg
                      className="h-4 w-4"
                      fill="none"
                  ...
                  <button
                    ...
                    className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-[var(--color-danger)] hover:bg-red-50"
                  >
                    <svg
                      className="h-4 w-4"
                      fill="none"
