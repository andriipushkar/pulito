...
type FilterKey = 'all' | 'orders' | 'promo' | 'other';
const PAGE_SIZE = 20;
...
export default function NotificationsPage() {
  ...
  // Fetch page 1 on mount + on filter change.
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      setIsLoading(true);
      ...
      const res = await apiClient.get<NotificationsResponse>(`/api/v1/me/notifications?${qs}`);
      if (!cancelled) {
    ...
    if (res.success && res.data) {
      ...
      setData((prev) =>
        prev
          ? {
              ...prev,
              notifications: [...prev.notifications, ...fresh.notifications],
              total: fresh.total,
  ...
  const hasMore = data ? data.notifications.length < data.total : false;
          ...
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
              filter === f.key
                ...
                : 'bg-[var(--color-bg)] text-[var(--color-text-secondary)] border border-[var(--color-border)] hover:bg-[var(--color-bg-secondary)]'
            }`}
          >
            {f.label}
          </button>
      ...
      {hasMore && (
        <div className="mt-6 flex justify-center">
          <button
            ...
            className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] px-6 py-2 text-sm font-medium hover:bg-[var(--color-bg-secondary)] disabled:opacity-50"
          >
            {isLoadingMore
              ? 'Завантаження…'
              : `Показати ще (${data ? data.total - data.notifications.length : 0})`}
          </button>
        </div>
      )}
    </div>
