...
export default function ReferralPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get<ReferralStats>('/api/v1/me/referral', {
          ...
          <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-red-50 text-red-500">
            ...
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M12 9v3.75m0-10.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285zM12 15.75h.007v.008H12v-.008z"
              />
            </svg>
          </div>
  ...
  const conversionRate =
    stats.totalReferred > 0 ? ((stats.convertedCount / stats.totalReferred) * 100).toFixed(0) : '0';
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-[var(--color-border)]/60 bg-[var(--color-bg)] p-4 text-center">
          ...
          <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
            Надішліть друзям ваше реферальне посилання
          </p>
        </div>
        <div className="rounded-2xl border border-[var(--color-border)]/60 bg-[var(--color-bg)] p-4 text-center">
          <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-bg-secondary)] text-sm font-bold text-[var(--color-text)] shadow-sm">
          ...
          <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
            Друг створює акаунт за вашим посиланням
          </p>
        </div>
        <div className="rounded-2xl border border-[var(--color-border)]/60 bg-[var(--color-bg)] p-4 text-center">
          <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-bg-secondary)] text-sm font-bold text-[var(--color-text)] shadow-sm">
          ...
          <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
            Ви отримуєте бонуси після першого замовлення друга
          </p>
        </div>
      </div>
          ...
          <p className="flex items-center gap-2 text-sm font-semibold text-[var(--color-text)]">
            ...
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.54a4.5 4.5 0 00-6.364-6.364L4.757 8.116A4.5 4.5 0 003 11.625"
              />
            </svg>
