...
export default function SettingsPage() {
  ...
  const handleDataExport = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setExportError('Не вдалося сформувати експорт. Спробуйте пізніше.');
        return;
      }
      ...
      setExportError('Помилка мережі під час експорту даних.');
    } finally {
  ...
  useEffect(() => {
    apiClient
      .get<{ hasGoogle: boolean; hasPassword: boolean }>('/api/v1/me/account/google')
      .then((res) => {
  ...
  const handleDeleteAccount = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Hold the success message long enough to be readable, then
        // log out + redirect to home. Keep isDeleting=true so the button
        // stays disabled and the user can't double-click during the delay.
        setTimeout(async () => {
          await logout();
          router.push('/');
        }, 2000);
  ...
  }, [deleteConfirmText, deletePassword, logout, router]);
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        ...
        title="Експорт ваших даних"
      >
        <div className="max-w-md space-y-3">
          <p className="text-sm text-[var(--color-text-secondary)]">
            Завантажте копію усіх ваших персональних даних у форматі JSON: профіль, адреси,
            замовлення та списки бажань.
          </p>
          {exportError && (
            <div className="rounded-xl bg-red-50 px-3 py-2 text-sm text-[var(--color-danger)]">
              {exportError}
            </div>
          )}
          <Button variant="outline" onClick={handleDataExport} isLoading={isExporting}>
      ...
      >
        <div className="max-w-md space-y-4">
          <p className="text-sm text-[var(--color-text-secondary)]">
            Ця дія незворотна. Усі ваші персональні дані буде анонімізовано, а доступ до акаунту —
            припинено. Історія замовлень збережеться в знеособленому вигляді.
          </p>
