...
export default function WebhooksPage() {
  ...
  const create = async () => {
    if (!draft.name || !draft.url) {
      toast.error('Заповніть назву та URL');
      return;
    }
  ...
  const toggleActive = async (id: number, isActive: boolean) => {
    ...
    if (!res.success) toast.error(res.error || 'Помилка оновлення');
    fetchAll();
  };

  const remove = async (id: number) => {
    if (!confirm('Видалити webhook?')) return;
    const res = await apiClient.delete(`/api/v1/admin/webhooks/${id}`);
    if (!res.success) toast.error(res.error || 'Помилка видалення');
    fetchAll();
  };
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-[var(--color-text-secondary)]">
          Зовнішні URL-и, на які платформа надсилає події (нове замовлення, оплата тощо).
          Підпис у header X-Webhook-Signature — HMAC-SHA256 від тіла з ключем секрета.
        </p>
      </div>

      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
        <h3 className="mb-2 text-sm font-semibold">Створити webhook</h3>
        ...
        <p className="mt-3 mb-1 text-xs font-semibold text-[var(--color-text-secondary)]">Події</p>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                    ...
                    {last && (
                      <p
                        className={`mt-2 text-xs ${
                          lastOk ? 'text-emerald-700' : 'text-red-700'
                        }`}
                      >
                        Остання спроба: {last.statusCode ?? 'error'}{' '}
                        {new Date(last.createdAt).toLocaleString('uk-UA')}
                        {last.error && ` — ${last.error}`}
                      </p>
                    )}
                    <details className="mt-2 text-xs text-[var(--color-text-secondary)]">
                      <summary className="cursor-pointer">Показати секрет</summary>
                  ...
                  <div className="flex shrink-0 gap-2">
                    ...
                    >
                      {wh.isActive ? 'Вимкнути' : 'Увімкнути'}
                    </Button>
                    <button
