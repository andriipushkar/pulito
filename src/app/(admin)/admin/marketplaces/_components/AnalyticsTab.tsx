...
function DeltaBadge({ current, previous }: { current: number; previous: number | undefined }) {
  const t = useTranslations('admin.analyticsTab');
  ...
  return (
    <span
      title={t('previousTitle', {
        delta: `${current >= previous ? '+' : ''}${rounded}`,
        prev: previous,
      })}
...
export function AnalyticsTab() {
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .then((res) => {
        ...
        else toast.error(res.error || t('loadFailed'));
      })
      .finally(() => {
  ...
  const aggregateSeries = (
    ...
    const keyFor = (iso: string): { key: string; label: string } => {
      ...
      return { key, label: t('weekLabel', { date: key.slice(5) }) };
    };
    for (const r of rows) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <span className="text-sm text-[var(--color-text-secondary)]">{t('periodLabel')}</span>
        <select
          value={period}
        ...
        >
          ...
          <option value="365d">{t('days365')}</option>
        </select>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div
          ...
          title={t('profitTitle')}
        >
          <p className="text-xs text-green-800">{t('profit')}</p>
          <p className="mt-1 text-2xl font-bold text-green-700">
            {formatCurrency(data.totals.netRevenue)}
          </p>
          {data.totals.commission > 0 && (
            <p className="mt-0.5 text-[10px] text-green-700">
              {t('commissionLine', { amount: formatCurrency(data.totals.commission) })}
            </p>
          )}
        </div>
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
          <p className="text-xs text-[var(--color-text-secondary)]">{t('avgOrder')}</p>
      ...
      {data.totals.commission === 0 && data.totals.revenue > 0 && (
        <div className="rounded-[var(--radius)] border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
          {t.rich('noCommissionNote', { b: (c) => <strong>{c}</strong> })}
        </div>
      )}

      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
        <p className="mb-3 text-sm font-semibold">{t('byMarketplace')}</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--color-border)] text-left text-xs text-[var(--color-text-secondary)]">
                ...
                <th className="px-2 py-2 text-right">{t('orders')}</th>
                <th className="px-2 py-2 text-right">{t('revenue')}</th>
                <th className="px-2 py-2 text-right" title={t('commissionColTitle')}>
                  {t('colCommission')}
                </th>
                <th className="px-2 py-2 text-right" title={t('profitColTitle')}>
                  {t('colProfit')}
                </th>
                <th className="px-2 py-2 text-right">{t('colAvg')}</th>
                <th className="px-2 py-2 text-right">{t('colPublished')}</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
        ...
        {series.length === 0 ? (
          <p className="text-xs text-[var(--color-text-secondary)]">{t('noOrdersPeriod')}</p>
        ) : (
          ...
          return (
            ...
            >
              <p className="mb-3 text-sm font-semibold">
                {m.icon} {t('top5', { name: m.name })}
              </p>
              {top.length === 0 ? (
                <p className="text-xs text-[var(--color-text-secondary)]">{t('noOrders')}</p>
              ) : (
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-[var(--color-border)] text-left text-[var(--color-text-secondary)]">
                      <th className="px-2 py-1.5">{t('colProduct')}</th>
