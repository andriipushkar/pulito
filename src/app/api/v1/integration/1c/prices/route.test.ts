...
describe('POST /api/v1/integration/1c/prices', () => {
  ...
  it('updates prices successfully', async () => {
    ...
    vi.mocked(updatePricesFrom1C).mockResolvedValue({ processed: 1, failed: 0, errors: [] });
  ...
  it('returns 500 on error', async () => {
    vi.mocked(oneCPriceUpdateSchema.safeParse).mockImplementation(() => { throw new Error('fail'); });
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
