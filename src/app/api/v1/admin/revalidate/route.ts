...
export const POST = withRole(
  ...
  try {
    ...
    // Revalidate by tag — calls Next.js's tag cache invalidation. The
    // previous implementation mapped tags to paths ("/product:123") which
    // silently no-op'd because no such route exists.
    if (tags && Array.isArray(tags)) {
      for (const tag of tags) {
        // Next.js 16+ requires a cache profile argument. 'default' invalidates
        // the standard Data Cache entry tagged with this tag.
