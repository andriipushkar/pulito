...
describe('POST /api/v1/admin/publications/[id]/publish', () => {
  ...
  it('publishes on success', async () => {
    vi.mocked(prisma.publication.updateMany).mockResolvedValue({ count: 1 } as any);
  ...
  it('returns PublicationError status code when prisma update fails', async () => {
    ...
    vi.mocked(prisma.publication.updateMany).mockRejectedValue(new PublicationError('not found'));
    const req = new Request('http://localhost', { method: 'POST' });
    const res = await POST(req as any, mockCtx as any);
    expect(res.status).toBe(400);
  });
  ...
  it('returns 500 on error', async () => {
    vi.mocked(prisma.publication.updateMany).mockRejectedValue(new Error('fail'));
    const req = new Request('http://localhost', { method: 'POST' });
    const res = await POST(req as any, mockCtx as any);
