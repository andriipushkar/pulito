...
export const GET = withRole(
  ...
  try {
    ...
    // Cheap path for the per-tab unread-badge polling — skips the heavy
    // findMany+normalize roundtrip and returns just the count. Multiple
    // browser tabs polling every 60s used to each pull the full message
    // list (~100KB+ on busy shops); now it's a single COUNT(*).
