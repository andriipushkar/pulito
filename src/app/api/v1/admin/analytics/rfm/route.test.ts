...
describe('GET /api/v1/admin/analytics/rfm', () => {
  ...
  it('returns RFM segments on success', async () => {
    ...
    (prisma.order.groupBy as any).mockResolvedValue([
      ...
      { userId: 2, _count: { id: 1 }, _sum: { totalAmount: 200 }, _max: { createdAt: new Date(now.getTime() - 60 * 24 * 60 * 60 * 1000) } },
    ]);

    const req = new Request('http://localhost/api/v1/admin/analytics/rfm?days=90');
  ...
  it('returns 500 on error', async () => {
    ...
    const req = new Request('http://localhost/api/v1/admin/analytics/rfm');
    const res = await GET(req);
