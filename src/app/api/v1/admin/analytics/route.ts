  ...
  try {
    ...
    switch (type) {
      ...
      case 'clients': {
        ...
        const wgUserIds = wholesaleGroups
          .map((g) => g.userId)
        ...
        const groupStats: Record<
          number,
          { orders: number; revenue: number; customers: Set<number> }
        > = {};
      ...
      case 'dashboard': {
        ...
        if (revChange !== null) {
          if (revChange > 10)
            insights.push(`Виручка зросла на ${revChange}% порівняно з попереднім періодом`);
          else if (revChange < -10)
            insights.push(
              `Виручка впала на ${Math.abs(revChange)}% порівняно з попереднім періодом`,
            );
        }
        const ordChange = pctChange(totalOrders, prevTotalOrders);
        if (ordChange !== null && ordChange < -15)
          insights.push(`Кількість замовлень зменшилась на ${Math.abs(ordChange)}%`);
        if (pendingOrders > 5) insights.push(`${pendingOrders} замовлень очікують обробки`);
        if (lowStockCount > 0)
          insights.push(`${lowStockCount} товарів з критично низьким залишком (< 5 шт)`);

        const usersChange = pctChange(newUsers, prevNewUsers);
        if (usersChange !== null && usersChange > 20)
          insights.push(`Приплив нових користувачів зріс на ${usersChange}%`);
