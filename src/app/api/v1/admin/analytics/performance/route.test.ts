...
describe('GET /api/v1/admin/analytics/performance', () => {
  ...
  it('returns metrics on success', async () => {
    vi.mocked(getAggregatedMetrics).mockResolvedValue({ avg: 100 });
  ...
  it('passes route filter parameter', async () => {
    vi.mocked(getAggregatedMetrics).mockResolvedValue({ avg: 50 });
  ...
  it('clamps days to min 1, max 90', async () => {
    vi.mocked(getAggregatedMetrics).mockResolvedValue({ avg: 10 });
