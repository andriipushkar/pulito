...
describe('POST /api/v1/admin/blog/categories', () => {
  ...
  it('creates category on success', async () => {
    (createBlogCategorySchema.safeParse as any).mockReturnValue({ success: true, data: { name: 'New Cat' } });
  ...
  it('returns 422 on validation error', async () => {
    (createBlogCategorySchema.safeParse as any).mockReturnValue({ success: false, error: { issues: [{ message: 'Invalid' }] } });
  ...
  it('returns 500 on error', async () => {
    (createBlogCategorySchema.safeParse as any).mockReturnValue({ success: true, data: { name: 'Cat' } });
