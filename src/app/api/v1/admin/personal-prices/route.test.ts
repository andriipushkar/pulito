...
describe('GET /api/v1/admin/personal-prices', () => {
  ...
  it('returns personal prices on success', async () => {
    vi.mocked(personalPriceFilterSchema.safeParse).mockReturnValue({
      success: true,
...
describe('POST /api/v1/admin/personal-prices', () => {
  ...
  it('creates personal price on success', async () => {
    vi.mocked(createPersonalPriceSchema.safeParse).mockReturnValue({
      success: true,
      data: { userId: 1, productId: 1, price: 50 },
    } as any);
  ...
  it('returns 400 on POST validation error', async () => {
    vi.mocked(createPersonalPriceSchema.safeParse).mockReturnValue({
      success: false,
      error: { issues: [{ message: 'bad price' }] },
    } as any);
    const req = new NextRequest('http://localhost', {
      method: 'POST',
