...
vi.mock('@/services/campaign', () => ({
  ...
  CampaignError: class CampaignError extends Error { statusCode: number; constructor(msg: string, code: number) { super(msg); this.statusCode = code; } },
}));
...
describe('GET /api/v1/admin/campaigns', () => {
  ...
  it('returns campaign rules on success', async () => {
    ...
    const req = new Request('http://localhost/api/v1/admin/campaigns');
...
describe('POST /api/v1/admin/campaigns', () => {
  ...
  it('creates campaign rule on success', async () => {
    (createCampaignRuleSchema.safeParse as any).mockReturnValue({ success: true, data: { name: 'New' } });
  ...
  it('returns 422 on validation error', async () => {
    (createCampaignRuleSchema.safeParse as any).mockReturnValue({ success: false, error: { issues: [{ message: 'Invalid' }] } });
  ...
  it('returns 500 on error', async () => {
    (createCampaignRuleSchema.safeParse as any).mockReturnValue({ success: true, data: { name: 'X' } });
