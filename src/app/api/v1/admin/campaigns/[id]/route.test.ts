...
describe('PATCH /api/v1/admin/campaigns/[id]', () => {
  ...
  it('updates campaign rule on success', async () => {
    (updateCampaignRuleSchema.safeParse as any).mockReturnValue({ success: true, data: { name: 'Updated' } });
  ...
  it('returns 500 on error', async () => {
    (updateCampaignRuleSchema.safeParse as any).mockReturnValue({ success: true, data: { name: 'X' } });
