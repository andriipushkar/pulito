...
vi.mock('@/services/jobs/auto-tracking', () => ({ autoTrackDeliveries: vi.fn() }));

import { POST } from './route';
import { autoTrackDeliveries } from '@/services/jobs/auto-tracking';

describe('POST src/app/api/v1/cron/auto-tracking', () => {
  ...
  it('returns 500 on error', async () => {
    vi.mocked(autoTrackDeliveries).mockRejectedValue(new Error('fail'));
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer test-app-secret' },
    });
