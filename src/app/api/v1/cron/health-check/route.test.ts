...
vi.mock('@/services/jobs/health-check', () => ({ runHealthChecks: vi.fn() }));

import { POST } from './route';
import { runHealthChecks } from '@/services/jobs/health-check';

describe('POST src/app/api/v1/cron/health-check', () => {
  ...
  it('returns 500 on error', async () => {
    vi.mocked(runHealthChecks).mockRejectedValue(new Error('fail'));
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer test-app-secret' },
    });
